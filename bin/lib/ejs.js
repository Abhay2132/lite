...
function engine({
	...
	ejsCache.set(
		baseLayout,
		ejs.compile(
			fs.readFileSync(getLayoutPath(baseLayout)).toString(),
			ejsOptions
		)
	);
	function renderer(filepath, options, callback) {
		...
		if (layout != baseLayout) {
			!ejsCache.has(layout) &&
				ejsCache.set(
					layout,
					ejs.compile(fs.readFileSync(layout).toString(), ejsOptions)
				);
		}
		...
		try {
			if (useCache) {
				...
				return ejs.renderFile(layout, renderOpts, ejsOptions, (err, str) =>
					callback(err, injectBase(str, base))
				);
			}
		} catch (e) {
			err = e;
